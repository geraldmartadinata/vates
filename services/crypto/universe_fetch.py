...
async def fetch_universe(
    ...
    try:
        comp_result = await fetch_historical(
            session, COMPOSITE, period="2y", force_fetch=False
        )
    ...
    logger.info(
        "Fetch universe done: %d tickers fetched, %d errors",
        ok_count, len(errors),
    )
    return {
        "results": results,
...
if __name__ == "__main__":
    # Quick smoke: fetch 3 tickers + composite (manual, no DB session)
